...
namespace Cactus.MSSQLService.CMS
{
    public class ColumnService : IColumnService
	{
        ...
        public bool IsUseColumnName(string columnName, int ignoreId)
        {
            ...
            {
                int i = conn.Query<int>("SELECT top 1 c.Column_Id FROM cms_column as c WHERE c.ColumnName=@columnName and c.Column_Id not in (@ignoreId) ", new { columnName = columnName, ignoreId = ignoreId }).SingleOrDefault();
                //是否存在
