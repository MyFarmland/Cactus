...
namespace Cactus.MSSQLService.CMS
{
    public class TempPageService : ITempPageService
    {
        ...
        public bool IsUseTempName(string tempName, int ignoreId)
        {
            using (IDbConnection conn = SqlString.GetSqlConnection())
            {
                int i = conn.Query<int>("SELECT top 1 c.TempPage_Id FROM cms_temppage as c WHERE c.TempName=@tempName and c.TempPage_Id not in (@ignoreId) ", new { tempName = tempName, ignoreId = ignoreId }).SingleOrDefault();
                if (i > 0) { return true; } else { return false; }
            }
        }
    }
