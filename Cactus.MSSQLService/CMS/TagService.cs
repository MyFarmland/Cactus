...
namespace Cactus.MSSQLService.CMS
{
    public class TagService : ITagService
    {
        ...
        public bool IsUseTagName(string tagName, int tagId)
        {
            ...
            {
                int i = conn.Query<int>("SELECT top 1 c.Tag_Id FROM cms_tag as c WHERE c.TagName==@tagName and c.Tag_Id not in (@tagId) ", new { tagName = tagName, tagId = tagId }).SingleOrDefault();
